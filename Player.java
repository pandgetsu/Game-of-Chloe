...
//          }
    ...
    public void render(Graphics g) 
    {
        ...
        if(isShield)
        {
           // g.drawImage(Assets.shield, (int) myHandler.getInstance().getEntityHandler().getMainCharacter().getMyX(), (int) myHandler.getInstance().getEntityHandler().getMainCharacter().getMyY(), null);
             g.drawImage(Assets.shield, (int) getMyX() - 16, (int) getMyY() - 16, null);
        }
    ...
    private BufferedImage getCurrentAnimation()
    {
        ...
        {
            //return last.getCurrentSprite();
            if(last == downWalk)
                return standing.getSprites()[0];
                else
                    if(last == rightWalk)
                        return standing.getSprites()[1];
                        else
                            if(last == upWalk)
                                return standing.getSprites()[2];
                                else
                                    if(last == leftWalk)
                                        return standing.getSprites()[3];
            return standing.getSprites()[0];
